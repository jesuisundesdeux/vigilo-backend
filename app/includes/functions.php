...
function tweetToken($token)
{
  ...
  $citynamehashtag = '#' . str_replace(['-', ' '], '', $cityname);
...
function generategroups($filter = ['distance' => 500, 'fdistance' => 0, 'fcategorie' => 0, 'faddress' => 0])
{
  global $db;
...
function isAllowed()
{
  global $page_name;
  global $menu;
  return isset($page_name)
      && isset($_SESSION['role'])
      && in_array($_SESSION['role'], $menu[$page_name]['access']);
}

function isSASS()
{
  global $config;
  return isset($config['SAAS_MODE'])
      && $config['SAAS_MODE'];
}

/**
 * Helper for sending recurrent error.
 * Making messages coherent against HTTP code
 * @param int $code=404 HTTP error code number, default : 404 (not found)
 * @param string $msg=null message enclose in formated string, default : "Not found" (404)
 * @param int|mixed $exit=1 if integer exit current script file with code equal to $exit
 */
function httpError($code = 404, $msg = null, $exit = 1)
{
  if ($msg === null) {
    switch ($code) {
      case 401: $msg = 'Resource not allowed ; request aborted.';
        break;
      case 404: $msg = 'Not found.';
        break;
      default: $msg = 'Server misconfiguration.';
        break;
    }
  }
  http_response_code($code);
  printf('<pre style="font-size:1.25em"><b style="color:darkred">%d</b> - %s</pre>', $code, $msg);
  if (is_int($exit)) {
    exit($exit);
  }
}
...
/**
 * Check if current session is owned by a privileged user
 * @return boolean user has privileges or not
 */
function isAdmin()
{
  return
...
/**
 * Download remote file with curl and save it into $filePath.
 * /!\
 * /!\ Overwrite destination file if exists.
 * /!\
 */
function saveWebContent($url, $filePath)
{
  $outputFile = fopen($filePath, 'w');
  chmod($filePath, 0660);
  $curl = curl_init($url);
  curl_setopt($curl, CURLOPT_FILE, $outputFile);
  curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($curl, CURLOPT_USERAGENT, sprintf('User-Agent: Vigilo Backend Version/%s', BACKEND_VERSION));
  $data = curl_exec($curl);
  if (!$data) {
...
/**
 * Remove an entire directory tree, recursively files and folders;
 * so be careful with that :)
 * @param string $dir folder pathname to remove
 */
function rmrf(string $dir): void
{
  if (!is_dir($dir)) {
    throw new ErrorException('$dir must be an existing directory.');
  }
  $objects = array_diff(scandir($dir), ['.', '..']);
  foreach ($objects as $object) {
    $subject = $dir . DIRECTORY_SEPARATOR . $object;
    error_log($subject);
    if (is_dir($subject) && !is_link($subject)) {
      rmrf($subject);
    } else {
      unlink($subject);
    }
