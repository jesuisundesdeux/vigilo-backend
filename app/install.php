...
root@p-mpl-fenouil:~/git/vigilo-backend/app# vim install.php 
root@p-mpl-fenouil:~/git/vigilo-backend/app# fg
vim install.php
root@p-mpl-fenouil:~/git/vigilo-backend/app# cat install.php 
<?php
require_once('config/config.php');
...
if(!$db = mysqli_connect($config['MYSQL_HOST'],
  ...
  $query_installed = mysqli_query("SELECT * FROM obs_config WHERE config_param='vigilo_urlbase'");
  if(mysqli_num_rows($query_installed) != 0) {
    echo "Instance déjà configurée";
    deleteInstallFile();
  } 
  else { 
    if(!empty($_POST)) {
      ...
      echo "Configuration mise en place";
      deleteInstallFile();
